...
function [model]=sub_00_model_halfspace_dx5m_500_500()
   ...
   model.source_ii        = 350*1;   % 注意有限元中，源是放置在单元中  halfspace--(300,300)  surface wave--(450,350)
   model.source_jj        = 300*1;   % source to free surface in Z direction: (model.Nz-model.Npz1-model.Npz0+1-model.source_jj)*model.dz-(1/2)*model.dz
...
%  model.porous       = ones(model.Nz,model.Nx)*0.15;
 ...
 %   model.epsi_rela_s  = ones(model.Nz,model.Nx)*4; 
     ...
     model.Qp=ones(model.Nz,model.Nx)*20000;
     model.Qs=ones(model.Nz,model.Nx)*20000;
%      model.Qp=20000;
%      model.Qs=20000;      
