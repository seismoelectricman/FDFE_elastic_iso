...
function [model]=sub_00_model_fullspace_dx5m_500_500()
   ...
   model.source_ii        = 200*1;  % 注意有限元中，源是放置在单元中
   model.source_jj        = 250*1;  % source to free surface in Z direction: (model.Nz-model.Npz1-model.Npz0-model.source_jj+1)*model.dz-(1/2)*model.dz
   
   model.source_type      = 2;    
   
        model.source_Fx      = 1;   %   Fx 强度  (1) 1e6
        model.source_Fy      = 1;   %   Fz 强度  (2) 1e6
        model.source_fx      = 1;   %   fx 强度  (3) 1e6
        model.source_fy      = 1;   %   fz 强度  (4) 1e6
...
%         model.source_Cy      = 1;     %   Cy 强度  (6)
   ...
   model.PML_on           = 1;     % 1: PML on     0; PML off
    ...
    model.model_free_surface = 0;      % 0.不含自由地表 1. 含自由地表    影响PML设置是否吸收最上面的介质层               
    model.Npz1             = 0;       % 自由地表以上空气层的层数                                                                       
...
% 
%      model.Vs(1:model.interface_z,1:model.Nx)             = 2800;   % 4000
...
%      %%   注意坐标系 左为正x，上为正y（或z）  
% 
    model.interface_z=375;     %  注意有限元中，参数是赋值在单元中而不是节点上 比如此处取50，则表明第y方向 50和51是分界面
    model.interface_z0=400;    %  源在界面上：(model.interface_z-model.source_jj+(1/2))）*model.dz
                               %  源在界面下：((model.Nz-model.source_jj+(1/2))-model.interface_z)*model.dz
%     model.Vp(1:model.Nz,1:model.Nx)           = 4000; 

     model.Vp(1:model.interface_z,1:model.Nx)             = 4000;   % 4000
        model.Vp((model.interface_z+1):(model.interface_z0),1:(model.Nx))      = 5000; 
            model.Vp((model.interface_z0+1):(model.Nz),1:(model.Nx))      = 5000;
           
     model.Vs(1:model.interface_z,1:model.Nx)             = 2800;   % 4000
          model.Vs((model.interface_z+1):(model.interface_z0),1:(model.Nx))      = 3500; 
              model.Vs((model.interface_z0+1):(model.Nz),1:(model.Nx))      = 3500;
   %            model.Vs((model.interface_z0+1):model.Nz,1:(model.Nx))      = 3000;   % 1. 直接赋值为0行不通,给一个很小的值（例如1），可以计算。但反射波与解析解有误差
                              
     model.dens(1:model.interface_z,1:model.Nx)           = 2000;   % 2800
          model.dens((model.interface_z+1):(model.interface_z0),1:model.Nx)    = 2400;
                model.dens((model.interface_z0+1):(model.Nz),1:model.Nx)    = 2400;
     ...
     model.Molarity(1:model.interface_z,1:model.Nx)           = 0.1;
          model.Molarity((model.interface_z+1):(model.interface_z0),1:model.Nx)    = 0.01;
                model.Molarity((model.interface_z0+1):(model.Nz),1:model.Nx)    = 0.01;
                                                     
      model.porous(1:model.interface_z,1:model.Nx)           = 0.15; 
          model.porous((model.interface_z+1):(model.interface_z0),1:model.Nx)    = 0.25;
                model.porous((model.interface_z0+1):(model.Nz),1:model.Nx)    = 0.25;                                                    
...
%  model.porous       = ones(model.Nz,model.Nx)*0.15;
 ...
 %   model.epsi_rela_s  = ones(model.Nz,model.Nx)*4; 
     ...
     model.Qp=ones(model.Nz,model.Nx)*40;
     model.Qs=ones(model.Nz,model.Nx)*50; 
   %  model.Qp=20000;
   %  model.Qs=20000;      
...
%               model.dx0(ii)=1000;
     ...
     for jj=1:model.Nz
            ...
            if((jj >= (model.Npz0+1))&&(jj<model.Npz0+model.Npz1+1))
                 if model.model_free_surface == 1;
                    model.dz0(jj)=1000; 
%                     model.dz0(jj)=5;
                 else
...
%% █████████████████████-------------------- 3 声源频率、记录时间                   █████████████████████% 
     ...
     model.refer_omega=2*pi*model.f_c;
...
%           Cp=model.Vs(1);
      %     Cp=max(max(model.Vs(:,:)));
           Cp=min(min(model.Vs(:,:)));
           model.PML_m       =2; 
           model.PML_Rc      = 0.0001;    % 0.0001                                                                % default = 0.0001;  
